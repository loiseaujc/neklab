         ...
         !------------------------------------------------------------
          ...
          subroutine resolvent_matvec(self, vec_in, vec_out)
            ...
            block
               ...
               ! GMRES solver.
               call vec_out%zero() ; alpha = b%norm() ; call b%scal(1.0_dp / alpha)
               select type(vec_out)
               type is(nek_zvector)
                  call gmres(S, b, vec_out%re, info, options=opts)
               end select
               call vec_out%scal(cmplx(alpha, 0.0_dp, kind=dp))
            end block
